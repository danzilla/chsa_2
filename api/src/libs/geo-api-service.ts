...
export default class GeoApiService {
  ...
  public async fetchChsaResponseSet(
    queriedPoint: QueriedPoint
  ): Promise<ChsaResponseSet> {
    ...
    try {
      const response = await this.axi.get(
      ...
      logger.info(
        `successfully received chsa payload (${JSON.stringify(payload)})`
      );
      ...
      if (features.length === 0) {
        throw errorWithCode(
          `invalid longtitude and latitude for a BC location,\
        you can use google map to search for the correct and try again `,
          422
        );
      }
      ...
      const {
        properties: { CMNTY_HLTH_SERV_AREA_CODE, CMNTY_HLTH_SERV_AREA_NAME },
      } = features[0];
